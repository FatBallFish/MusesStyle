...
def conv2d(x, input_filters, output_filters, kernel, strides, mode='REFLECT'):
    ...
    with tf.variable_scope('conv'):
        ...
        # 使用convd2D内置的padding可以节约大量的计算时间
        # x_padded = tf.pad(x, [[0, 0], [int(kernel/2), int(kernel/2)],
...
def net(image, training):
    ...
    with tf.variable_scope('res1'):
        res1 = residual(conv3, 128, 1, 1)
    with tf.variable_scope('res2'):
        res2 = residual(res1, 128, 1, 1)
    with tf.variable_scope('res3'):
        res3 = residual(res2, 128, 1, 1)
    with tf.variable_scope('res4'):
        res4 = residual(res3, 128, 1, 1)
    with tf.variable_scope('res5'):
        res5 = residual(res4, 128, 1, 1)
